# -*- coding: utf-8 -*-
"""
Created on Thu Mar 19 11:52:43 2020

@author: lparry
"""
# install pyopencl from https://www.lfd.uci.edu/~gohlke/pythonlibs/#pyopencl
# this is required t o enable the opengl shared context
...
class TransientHeatSim(pyocl.OpenCLSimBase):
    ...
    def __init__(self, kernel:str, u0):
    ...
    def initialiseData(self, u0):
        ...
        transferFlag = mf.USE_HOST_PTR if self.myocl.hasSharedMemory() else mf.COPY_HOST_PTR

        #Upload data to the device

        self.u0 = cl.Buffer(self.myocl.context, mf.READ_WRITE | transferFlag, hostbuf=u0)

        #Allocate output buffers
        self.u1 = cl.Buffer(self.myocl.context, mf.READ_WRITE , u0.nbytes)
...
#u3 = u0.astype(np.float64)

# Create the OpenCL Transient Heat Kernel
heatsim = TransientHeatSim(kernel, u0)
heatsim.k   = 10.0    s# W/kgK
...
# Note that on shared memory devices (Intel) The result can be 
